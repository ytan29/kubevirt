...
var resourcePrefix = "DisplayDevice"
...
func NewDisplayAddressPool(displayDevices []v1.Display) *AddressPool {
	return NewAddressPool(v1.DisplayResourcePrefix, extractResources(displayDevices))
}
...
func (p *AddressPool) load(resourcePrefix string, resources []string) {
	for _, resource := range resources {
		log.Log.Warningf("===== loading for resource %s", resource)
		addressEnvVarName := util.ResourceNameToEnvVar(resourcePrefix, resource)
		log.Log.Infof("======================env var name: %s", addressEnvVarName)
...
func (p *AddressPool) Pop(resource string) (string, error) {
	...
	log.Log.Infof("*******Addresspool resouce : %s", resource)

	if len(addresses) > 0 {
		selectedAddress := addresses[0]
		log.Log.Infof("*******Addresspool addresses : %s", selectedAddress)

		for resourceName, resourceAddresses := range p.addressesByResource {
			log.Log.Infof("*******Addresspool resouce Name: %s", resourceName)
